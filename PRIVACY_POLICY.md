...
- We do **not** claim the product is "100% local." Wake word and speech-to-text are on-device. Everything after that follows the endpoints you configured. That was already true before 0.1.587; this release encrypts the Music Assistant speaker path on your LAN rather than leaving it as plaintext
...
Motion detection compares successive camera frames on the device to see whether anything changed. **There is no face detection and no face recognition** — DashVoice does not identify people.

By default, camera frames stay on the device and are not stored. If you enable **camera stream** (off unless you turn it on), the tablet serves JPEG frames on your local HTTP API (`/cam.jpg`) so *your* Frigate or Home Assistant can pull them. That is traffic you asked for, to a server you named — not a DashVoice cloud.
...
- **Your Music Assistant server** (if Sendspin is enabled) - encrypted speaker audio on your LAN
- **Your Loki / log endpoint** (if you enable remote logging)
- **Your Frigate or other consumer** of `/cam.jpg` (only if you enable camera stream)
...
| Music Assistant | Multiroom audio | Encrypted audio stream on your LAN (Sendspin) |
| Loki (optional) | Your log aggregation | Log lines you enabled |
| Camera stream (optional) | Frigate / HA stills | JPEG frames, only if you turn the stream on |
...
- No data is sent to Ecohash. Traffic leaves your LAN only if you pointed Home Assistant, TTS, Music Assistant, Loki, or camera stream at an address off-LAN
